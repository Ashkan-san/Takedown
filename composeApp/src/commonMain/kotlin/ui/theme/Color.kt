import androidx.compose.ui.graphics.Color

// COLORS
val white = Color(0xFFFFFF)
val almostWhite = Color(0xFFFDFCFF)
val almostBlack = Color(0xFF1A1C1E)
val lightGrey = Color(0xFFDFE2EB)
val darkGrey = Color(0xFF43474E)

val blue = Color(0xFF0B61A4)
val lightBlue = Color(0xFFD2E4FF)
val darkBlue = Color(0xFF001C37)

val red = Color(0xFFB72200)
val lightRed = Color(0xFFFFDAD3)
val darkRed = Color(0xFF3D0500)

// LIGHT THEME
val md_theme_light_primary = blue
val md_theme_light_onPrimary = white
val md_theme_light_primaryContainer = lightBlue
val md_theme_light_onPrimaryContainer = darkBlue

val md_theme_light_secondary = red
val md_theme_light_onSecondary = white
val md_theme_light_secondaryContainer = lightRed
val md_theme_light_onSecondaryContainer = darkRed

val md_theme_light_tertiary = Color(0xFF6E5E00)
val md_theme_light_onTertiary = white
val md_theme_light_tertiaryContainer = Color(0xFFFFE256)
val md_theme_light_onTertiaryContainer = Color(0xFF211B00)

val md_theme_light_error = red
val md_theme_light_errorContainer = lightRed
val md_theme_light_onError = white
val md_theme_light_onErrorContainer = darkRed

val md_theme_light_background = almostWhite
val md_theme_light_onBackground = almostBlack

val md_theme_light_surface = almostWhite
val md_theme_light_onSurface = almostBlack
val md_theme_light_surfaceVariant = lightGrey
val md_theme_light_onSurfaceVariant = darkGrey

val md_theme_light_outline = Color(0xFF73777F)
val md_theme_light_inverseOnSurface = Color(0xFFF1F0F4)
val md_theme_light_inverseSurface = Color(0xFF2F3033)
val md_theme_light_inversePrimary = Color(0xFFA0C9FF)
val md_theme_light_shadow = Color(0xFF000000)
val md_theme_light_surfaceTint = Color(0xFF0B61A4)
val md_theme_light_outlineVariant = Color(0xFFC3C6CF)
val md_theme_light_scrim = Color(0xFF000000)


// DARK THEME

val md_theme_dark_primary = Color(0xFFA0C9FF)
val md_theme_dark_onPrimary = Color(0xFF003259)
val md_theme_dark_primaryContainer = Color(0xFF00497F)
val md_theme_dark_onPrimaryContainer = Color(0xFFD2E4FF)
val md_theme_dark_secondary = Color(0xFFFFB4A4)
val md_theme_dark_onSecondary = Color(0xFF630E00)
val md_theme_dark_secondaryContainer = Color(0xFF8C1800)
val md_theme_dark_onSecondaryContainer = Color(0xFFFFDAD3)
val md_theme_dark_tertiary = Color(0xFFE5C500)
val md_theme_dark_onTertiary = Color(0xFF393000)
val md_theme_dark_tertiaryContainer = Color(0xFF534600)
val md_theme_dark_onTertiaryContainer = Color(0xFFFFE256)
val md_theme_dark_error = Color(0xFFFFB4AB)
val md_theme_dark_errorContainer = Color(0xFF93000A)
val md_theme_dark_onError = Color(0xFF690005)
val md_theme_dark_onErrorContainer = Color(0xFFFFDAD6)
val md_theme_dark_background = Color(0xFF1A1C1E)
val md_theme_dark_onBackground = Color(0xFFE3E2E6)
val md_theme_dark_surface = Color(0xFF1A1C1E)
val md_theme_dark_onSurface = Color(0xFFE3E2E6)
val md_theme_dark_surfaceVariant = Color(0xFF43474E)
val md_theme_dark_onSurfaceVariant = Color(0xFFC3C6CF)
val md_theme_dark_outline = Color(0xFF8D9199)
val md_theme_dark_inverseOnSurface = Color(0xFF1A1C1E)
val md_theme_dark_inverseSurface = Color(0xFFE3E2E6)
val md_theme_dark_inversePrimary = Color(0xFF0B61A4)
val md_theme_dark_shadow = Color(0xFF000000)
val md_theme_dark_surfaceTint = Color(0xFFA0C9FF)
val md_theme_dark_outlineVariant = Color(0xFF43474E)
val md_theme_dark_scrim = Color(0xFF000000)